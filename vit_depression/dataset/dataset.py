...
class DepressionDataset(Dataset):
    '''创建训练、验证或测试数据集，并在调用时加载参与者的特征'''

    def __init__(self,
                 root_dir,  # 数据集根目录
                 mode,  # 数据集模式：训练、验证或测试
                 use_mel_spectrogram=True,  # 是否使用梅尔频谱图
                 visual_with_gaze=True,  # 是否包含视觉特征和凝视信息
                 transform=None):  # 数据转换方式
        super(DepressionDataset, self).__init__()

        # 检查数据集模式，只允许训练、验证或测试模式
        # assert mode in ["train", "validation", "test"], \
        #     "Argument --mode could only be ['train', 'validation', 'test']"
        ...
        if mode == 'train' or mode == 'validation' or mode == 'test':
            self.IDs = np.load(os.path.join(self.root_dir, 'ID_gt.npy'))  # 参与者ID
            self.gender_gt = np.load(os.path.join(self.root_dir, 'gender_gt.npy'))  # 性别特征
            self.phq_binary_gt = np.load(os.path.join(self.root_dir, 'phq_binary_gt.npy'))  # PHQ二元分类结果
            self.phq_score_gt = np.load(os.path.join(self.root_dir, 'phq_score_gt.npy'))  # PHQ分数
            # 对于测试模式，可能没有 'phq_subscores_gt' 数据
            if mode != 'test':
                self.phq_subscores_gt = np.load(os.path.join(self.root_dir, 'phq_subscores_gt.npy'))  # PHQ子分数
    ...
    def __getitem__(self, idx):
        '''
        PyTorch创建数据集的基本函数，会在Dataloader中自动调用，根据self.IDs中的索引加载对应索引的特征数据
        参数:
            idx: int, self.IDs中参与者ID的索引
        返回:
            session: dict, 包含一个参与者/会话的所有提取特征和真实结果
        '''
        if torch.is_tensor(idx):
            idx = idx.tolist()
        ...
        # 获取视觉特征
        if self.visual_with_gaze:
            fkps_path = os.path.join(self.root_dir, 'facial_keypoints')  # 面部关键点路径
            gaze_path = os.path.join(self.root_dir, 'gaze_vectors')  # 凝视向量路径

            # 加载和创建最终的视觉特征，通过连接面部关键点和凝视向量
            fkps_file = np.sort(os.listdir(fkps_path))[idx]
            gaze_file = np.sort(os.listdir(gaze_path))[idx]
            fkps = np.load(os.path.join(fkps_path, fkps_file))
            gaze = np.load(os.path.join(gaze_path, gaze_file))
